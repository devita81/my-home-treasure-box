        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 rounded-lg border border-border bg-card shadow-sm overflow-hidden w-full">
            ...
            <div className="flex flex-col gap-0.5 px-3 py-1.5 border-t border-border">
              <label className="text-data font-medium uppercase tracking-wider text-muted-foreground">
                Período
              </label>
              <PeriodFilterButton
                from={periodFrom}
            ...
            {anyDateFilterActive ? (
              <button
                ...
                className="flex items-center justify-center gap-1.5 px-3 py-2 text-label font-semibold uppercase tracking-wide text-destructive bg-destructive/5 hover:bg-destructive/10 active:bg-destructive/15 transition-colors border-l border-t border-border"
        ...
        <Card>
          <CardContent className="p-3 sm:p-4 space-y-3">
            ...
            <div className="flex items-center gap-2 pt-1 border-t">
              <div className="flex items-center gap-1.5 text-label text-muted-foreground shrink-0">
                  ...
                  <TableHeader>
                    <TableRow>
                      ...
                      {pivot.monthLabels.map((m, i) => (
                        <TableHead key={i} className="text-right text-label whitespace-nowrap min-w-[72px] px-3">{m}</TableHead>
                      ))}
                      <TableHead className="text-right text-label font-semibold whitespace-nowrap pl-4 pr-6 min-w-[100px]">Total</TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {pivot.rows.map(r => (
                      ...
                      >
                        <TableCell className="sticky left-0 bg-card z-10 font-medium text-sm max-w-[260px] align-top">
                          <div className="flex flex-col leading-tight">
                            <span className="text-data uppercase tracking-wide text-muted-foreground font-medium truncate">
                              {r.cidade.toUpperCase() || '—'}
                            </span>
                            <span className="text-data font-semibold truncate">
                              {(r.rua || '—').toUpperCase()}
                            </span>
                        ...
                        {pivot.months.map(mk => {
                          ...
                          return (
                            <TableCell
                              key={mk}
                              className={cn(
                                'text-right text-label tabular-nums whitespace-nowrap px-3',
                      ...
                      {pivot.months.map(mk => {
                        ...
                        return (
                          <TableCell
                            key={mk}
                            className={cn(
                              'text-right text-label font-semibold tabular-nums whitespace-nowrap px-3',
                              v > 0 && 'text-emerald-600 dark:text-emerald-400',
                              v < 0 && 'text-red-600 dark:text-red-400',
                              v === 0 && 'text-muted-foreground/60'
                            )}
                ...
                <div className="min-w-0 pr-8">
                  {cidade && (
                    <div className="flex items-center gap-1.5 mb-1 min-w-0">
                      ...
                      <span className="text-label sm:text-data uppercase tracking-[0.12em] font-semibold text-muted-foreground truncate">
                        {cidade}
                      </span>
                    </div>
                  )}
                  <div className="text-data sm:text-base font-display font-semibold leading-tight truncate text-foreground">
                    {endereco}
                  </div>
                  <div className="flex items-center gap-1.5 mt-1 text-data text-muted-foreground">
          ...
          <div className="md:hidden min-w-0 overflow-hidden flex flex-col" style={{ maxHeight: 'calc(100dvh - 6rem)' }}>
            ...
            <Tabs defaultValue="grafico" className="flex flex-col min-h-0 flex-1">
              <TabsList className="grid grid-cols-4 h-8 mx-2 mt-2 shrink-0">
                <TabsTrigger value="grafico" className="text-data px-1">Gráfico</TabsTrigger>
                <TabsTrigger value="anos" className="text-data px-1">Anos</TabsTrigger>
                <TabsTrigger value="categorias" className="text-data px-1">Categ.</TabsTrigger>
                <TabsTrigger value="meses" className="text-data px-1">Meses</TabsTrigger>
              </TabsList>
              ...
              <div className="flex-1 min-h-0 overflow-y-auto px-2.5 pb-3 pt-2">
                <TabsContent value="grafico" className="mt-0 space-y-2">
                  <div className="rounded-md border bg-card p-1.5">
                    ...
                    <div className="flex items-center justify-center gap-3 pt-1 text-label text-muted-foreground">
                  ...
                  <div className="rounded-md border bg-card p-2 text-data space-y-1">
                    <div className="font-semibold text-data uppercase tracking-wide text-muted-foreground">Médias mensais</div>
                    ...
                    return (
                      <div className="rounded-md border bg-card p-2 text-data space-y-1">
                        <div className="flex items-center justify-between">
                          <div className="font-semibold text-data uppercase tracking-wide text-muted-foreground">
                            Último mês • {MONTHS[last.mes - 1]}/{String(last.ano).slice(2)}
                          </div>
                          {last.alugado && (
                            <Badge variant="secondary" className="h-4 px-1.5 text-label">Alugado</Badge>
                          )}
                        </div>
                        {last.locatario && (
                        ...
                        <div className="flex items-center justify-between pt-1 mt-1 border-t">
                          <span className="text-data text-muted-foreground">Receita</span>
                          <span className="tabular-nums font-semibold text-emerald-600 dark:text-emerald-400">{fmtBRLFull(receita)}</span>
                        </div>
                        <div className="flex items-center justify-between">
                          <span className="text-data text-muted-foreground">Despesa</span>
                        ...
                        <div className="flex items-center justify-between pt-0.5 border-t">
                          <span className="text-data font-semibold">Líquido</span>
                          <span
                            className={cn(
                ...
                <div key={r.id} className="rounded-lg border bg-card p-2.5 sm:p-3 min-w-0 overflow-hidden">
                  <div className="flex items-center justify-between gap-2 mb-2">
                    <div className="flex items-center gap-2">
                      <Badge variant="outline" className="text-data px-2 py-0 h-5">
                        {MONTHS[r.mes - 1]}/{r.ano}
                      </Badge>
                      {r.alugado && <Badge className="text-data px-2 py-0 h-5 bg-emerald-500/15 text-emerald-700 dark:text-emerald-400 border-emerald-500/30 hover:bg-emerald-500/15">Alugado</Badge>}
                    </div>
                    <div
                      className={cn(
                  ...
                  {r.locatario && (
                    <div className="text-label text-muted-foreground mb-2 truncate">
                      Locatário: <span className="text-foreground">{r.locatario}</span>
                    </div>
                  )}
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-3 gap-y-1 text-data sm:text-label min-w-0 overflow-hidden">
              ...
              return (
                <div className="min-w-0 pr-8">
                  ...
                  <div className="flex items-baseline gap-1.5 mb-1.5">
                    ...
                    <span className="text-label uppercase tracking-widest font-semibold text-muted-foreground">
                      {cidade || 'Período'}
                    </span>
                    <span className="text-muted-foreground/40 text-label">·</span>
                  ...
                  <div className="text-data font-semibold leading-tight break-words">
                    {endereco}
                  </div>
                </div>
              );
          ...
          <DialogHeader className="px-3 pt-3 pb-2.5 sticky top-0 bg-gradient-to-b from-background to-background/95 backdrop-blur z-10 border-b">
            ...
            {yearMonthDrilldown && (
              <div className="min-w-0 pr-8">
                ...
                <div className="text-data font-semibold leading-tight">
              ...
              <>
                ...
                <div className="grid grid-cols-3 gap-1.5 mb-2">
                  <div className="rounded-md bg-card border p-2">
                    <div className="text-label uppercase tracking-wide text-muted-foreground font-medium">Receita</div>
                    <div className="text-label font-semibold tabular-nums text-emerald-600 dark:text-emerald-400 mt-0.5">
                      {fmtBRL(yearMonthTotals.receita)}
                    </div>
                  </div>
                  <div className="rounded-md bg-card border p-2">
                    <div className="text-label uppercase tracking-wide text-muted-foreground font-medium">Despesa</div>
                    <div className="text-label font-semibold tabular-nums text-red-600 dark:text-red-400 mt-0.5">
                      {fmtBRL(yearMonthTotals.despesa)}
                    </div>
                  </div>
                  <div className="rounded-md bg-card border p-2">
                    <div className="text-label uppercase tracking-wide text-muted-foreground font-medium">Líquido</div>
                    <div className={cn(
                      'text-label font-semibold tabular-nums mt-0.5',
                ...
                {yearMonthRows.map((r, idx) => (
                  ...
                  >
                    <div className="min-w-0 flex-1">
                      <div className="text-label font-medium truncate">{r.label}</div>
                      <div className="text-data text-muted-foreground mt-0.5 flex items-center gap-1.5">
                        {r.alugado && (
                          <Badge className="text-label px-1.5 py-0 h-4 bg-emerald-500/15 text-emerald-700 dark:text-emerald-400 border-emerald-500/30 hover:bg-emerald-500/15">
                            Alugado
                          </Badge>
                        )}
...
function MoneyTooltip({ active, payload, label }: TooltipProps<number, string>) {
  ...
  return (
    <div className="rounded-lg border bg-background/95 backdrop-blur px-2.5 py-1.5 shadow-lg text-label">
...
function MiniStat({ label, value, tone }: { label: string; value: number; tone: 'positive' | 'negative' }) {
  return (
    <div className="rounded-md border bg-card px-2 py-1.5 sm:p-2 w-full max-w-full min-w-0 overflow-hidden block">
      <div className="text-label sm:text-data leading-none text-muted-foreground uppercase truncate">{label}</div>
      <div className={cn(
        'text-data sm:text-sm font-semibold tabular-nums mt-1 truncate leading-none text-left',
...
function MobileTotalCell({ label, value, tone }: { label: string; value: number; tone: 'positive' | 'negative' }) {
  return (
    <div className="min-w-0 px-2 py-1.5 border-r last:border-r-0">
      <div className="text-meta uppercase leading-none text-muted-foreground truncate">{label}</div>
      <div className={cn(
        'text-data font-bold tabular-nums leading-tight truncate mt-0.5',
        tone === 'positive' ? 'text-emerald-600 dark:text-emerald-400' : 'text-red-600 dark:text-red-400'
      )}>
        {fmtBRL(value)}
      </div>
    ...
    >
      <div className="flex items-center justify-between gap-2 min-w-0">
        <div className="flex items-center gap-1.5 min-w-0">
          <Badge variant="outline" className="h-4 px-1.5 text-label shrink-0">
            {MONTHS[row.mes - 1]}/{String(row.ano).slice(-2)}
          </Badge>
          {row.alugado && (
            <Badge className="h-4 px-1.5 text-meta shrink-0 bg-emerald-500/15 text-emerald-700 dark:text-emerald-400 border-emerald-500/30 hover:bg-emerald-500/15">
              Alugado
            </Badge>
          )}
          {row.locatario && <span className="text-label text-muted-foreground truncate min-w-0">{row.locatario}</span>}
        </div>
...
function MobileAmount({ label, value, tone, strong }: { label: string; value: number; tone: 'positive' | 'negative'; strong?: boolean }) {
  return (
    <div className="min-w-0">
      <div className="text-meta text-muted-foreground leading-none truncate">{label}</div>
      <div className={cn(
        'text-data tabular-nums leading-tight truncate',
...
function YearAggTable({ rows }: { rows: BalanceteRow[] }) {
  ...
  if (!years.length) return <p className="text-data text-muted-foreground text-center py-4">Sem dados.</p>;
  return (
    <div className="rounded-md border bg-card overflow-hidden">
      <div className="grid grid-cols-[2.6rem_1fr_1fr_1fr] gap-1 px-1.5 py-1 bg-muted/40 text-meta uppercase tracking-wide text-muted-foreground font-semibold">
        <span>Ano</span>
      ...
      {years.map(([ano, agg], idx) => (
        <div key={ano} className={cn('grid grid-cols-[2.6rem_1fr_1fr_1fr] gap-1 px-1.5 py-1 items-center text-data tabular-nums', idx > 0 && 'border-t')}>
          <span className="font-semibold">{ano}<span className="text-muted-foreground text-meta ml-0.5">·{agg.meses}m</span></span>
...
function CategoryAggTable({ totals }: { totals: { aluguel: number; condominio: number; iptu: number; taxa: number; outras: number; reembolso: number; liquido: number } }) {
  ...
  if (!items.length) return <p className="text-data text-muted-foreground text-center py-4">Sem dados.</p>;
      ...
      {items.map((i, idx) => {
        ...
        return (
          <div key={i.label} className={cn('px-2 py-1.5 min-w-0', idx > 0 && 'border-t')}>
            <div className="flex items-center justify-between gap-2 min-w-0">
              <span className="text-data text-muted-foreground truncate">{i.label}</span>
              <span className={cn(
                'text-data font-semibold tabular-nums whitespace-nowrap shrink-0',
  ...
  return (
    <div className="w-full max-w-full min-w-0 overflow-hidden space-y-3">
      ...
      {(row.alugado || row.locatario) && (
        <div className="flex items-center gap-1.5 text-data min-w-0">
          {row.alugado && (
            <Badge className="text-label px-1.5 py-0 h-4 bg-emerald-500/15 text-emerald-700 dark:text-emerald-400 border-emerald-500/30 hover:bg-emerald-500/15 shrink-0">
              Alugado
            </Badge>
          )}
          {row.locatario && (
      ...
      {receitaItems.length > 0 && (
        <div className="rounded-md border overflow-hidden">
          <div className="bg-emerald-500/10 px-2 py-1 text-label uppercase tracking-wide font-semibold text-emerald-700 dark:text-emerald-400">
            Receitas
          </div>
      ...
      {despesaItems.length > 0 && (
        <div className="rounded-md border overflow-hidden">
          <div className="bg-red-500/10 px-2 py-1 text-label uppercase tracking-wide font-semibold text-red-700 dark:text-red-400">
            Despesas
          </div>
      ...
      <button
        type="button"
        onClick={onOpenFull}
        className="w-full flex items-center justify-center gap-1 py-1.5 px-2 rounded-md text-data font-medium text-primary hover:bg-primary/5 active:bg-primary/10 transition-colors border"
      >
...
function SummaryRow({
  ...
  return (
    <div className="flex items-center justify-between gap-2 px-2 py-1.5 min-w-0">
      <span className={cn('text-data uppercase tracking-wide text-muted-foreground truncate', bold && 'font-semibold text-foreground')}>
        {label}
      </span>
      <span
        className={cn(
          'text-label tabular-nums whitespace-nowrap shrink-0',
          bold ? 'font-bold' : 'font-semibold',
          tone === 'positive' ? 'text-emerald-600 dark:text-emerald-400' : 'text-red-600 dark:text-red-400'
        )}
      >
...
function DetailRow({
  ...
  return (
    <div className={cn('flex items-center justify-between gap-2 px-2 py-1.5 min-w-0', divider && 'border-t')}>
      <span className="text-data text-muted-foreground truncate min-w-0">{label}</span>
      <span
        className={cn(
          'text-label font-medium tabular-nums whitespace-nowrap shrink-0',
...
function Line2({ label, value, positive }: { label: string; value: number; positive?: boolean }) {
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-[minmax(0,1fr)_minmax(0,6.75rem)] items-baseline gap-0.5 sm:gap-2 min-w-0 overflow-hidden">
      ...
      <span className={cn(
        'tabular-nums min-w-0 text-left sm:text-right leading-tight whitespace-nowrap text-sm sm:text-label',
        positive ? 'text-emerald-600 dark:text-emerald-400' : 'text-red-600 dark:text-red-400'
      )}>
        {fmtBRL(value)}
      </span>
...
function BreakdownLine({
  ...
  return (
    <div className="flex items-baseline justify-between gap-2">
      ...
      <span className={cn(
        'tabular-nums text-data sm:text-label',
        ...
        >
          <div className="flex items-center gap-2 sm:gap-3 p-3 sm:p-4">
            ...
            <div className="flex flex-col min-w-0 flex-1">
              {last12.periodoLabel && (
                <span className="text-label sm:text-sm text-foreground tabular-nums font-semibold">
                  {last12.periodoLabel}
                </span>
              )}
            </div>
            ...
            <div className="ml-auto flex items-center gap-2 sm:gap-4 text-data sm:text-sm flex-wrap justify-end shrink-0">
              <div className="flex items-baseline gap-1 tabular-nums">
                <span className="text-label sm:text-data text-muted-foreground font-medium">R</span>
                <span className="text-emerald-600 dark:text-emerald-400 font-semibold">{fmtBRL(last12.receita)}</span>
              </div>
              <div className="flex items-baseline gap-1 tabular-nums">
                <span className="text-label sm:text-data text-muted-foreground font-medium">D</span>
                <span className="text-red-600 dark:text-red-400 font-semibold">{fmtBRL(last12.despesa)}</span>
              </div>
              <div className="flex items-baseline gap-1 tabular-nums">
                <span className="text-label sm:text-data text-muted-foreground font-medium">L</span>
          ...
          <div className="border-t px-3 sm:px-4 py-3 space-y-2.5">
            <div className="grid grid-cols-2 gap-2 sm:gap-3">
              ...
              <div className="rounded-md border border-emerald-500/20 bg-emerald-500/5 p-2 sm:p-2.5 space-y-1">
                <div className="text-label uppercase tracking-wide font-bold text-emerald-700 dark:text-emerald-400">
                  Receitas
                </div>
              ...
              <div className="rounded-md border border-red-500/20 bg-red-500/5 p-2 sm:p-2.5 space-y-1">
                <div className="text-label uppercase tracking-wide font-bold text-red-700 dark:text-red-400">
                  Despesas
                </div>
            ...
            <div className="rounded-md border bg-muted/30 px-3 py-2 flex items-center justify-between gap-2">
              <span className="text-data sm:text-sm uppercase tracking-wide font-bold text-foreground">
                Resultado líquido
              </span>
              <div className="flex items-baseline gap-2 sm:gap-3 flex-wrap justify-end">
                {totalReembolso > 0 && (
                  <span className="text-label sm:text-data text-muted-foreground tabular-nums hidden sm:inline">
          ...
          <Card key={y.ano} className="overflow-hidden">
            ...
            >
              <div className="flex items-center gap-2 sm:gap-3 p-3 sm:p-4">
                ...
                <div className="ml-auto flex items-center gap-2 sm:gap-4 text-data sm:text-sm flex-wrap justify-end">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-1.5 p-1">
                  {Array.from({ length: 12 }).map((_, i) => {
                    ...
                    return (
                      ...
                      >
                        <div className="text-label uppercase tracking-wide text-muted-foreground font-semibold">
                          {MONTHS[i]}
                        </div>
                        {empty ? (
                          <div className="text-label font-semibold tabular-nums text-muted-foreground/50 mt-0.5">
                            —
                          </div>
                        ) : (
                          <div className="mt-0.5 space-y-0 leading-tight">
                            <div className="flex items-baseline justify-between gap-1 tabular-nums">
                              <span className="text-meta text-muted-foreground/80 font-medium">R</span>
                              <span className="text-data font-medium text-emerald-600 dark:text-emerald-400">
                                {fmtBRL(m.receita)}
                              </span>
                            </div>
                            <div className="flex items-baseline justify-between gap-1 tabular-nums">
                              <span className="text-meta text-muted-foreground/80 font-medium">D</span>
                              <span className="text-data font-medium text-red-600 dark:text-red-400">
                                {fmtBRL(m.despesa)}
                              </span>
                            </div>
                            <div className="flex items-baseline justify-between gap-1 tabular-nums border-t border-border/50 pt-0.5 mt-0.5">
                              <span className="text-meta text-muted-foreground font-semibold">L</span>
                              <span className={cn(
    ...
    <div className="rounded-lg border bg-card overflow-hidden">
      <div className="flex items-stretch">
        ...
        >
          <div className="min-w-0 flex-1">
            {(row.cidade || '').trim() && (
              <div className="text-label font-medium uppercase tracking-wide text-muted-foreground leading-tight">
                {(row.cidade || '').toUpperCase()}
              </div>
            )}
            <div className="text-label font-semibold leading-snug break-words uppercase">
              {(row.rua || '—').toUpperCase()}
            </div>
            {(row.numero || row.apartamento || row.complemento) && (
              <div className="text-data text-muted-foreground leading-snug break-words uppercase">
          ...
          <div className="flex flex-col items-end gap-0.5 shrink-0 pt-0.5">
            <div
              className={cn(
                'text-label font-semibold tabular-nums whitespace-nowrap',
                row.total > 0
                  ? 'text-emerald-600 dark:text-emerald-400'
                  : row.total < 0
                  ? 'text-red-600 dark:text-red-400'
      ...
      {open && (
        <div className="border-t bg-muted/20 px-2 py-2 space-y-2">
                      ...
                      >
                        ...
                        {empty ? (
                          ...
                          <div className="mt-0.5 space-y-0 leading-tight">
                            ...
                            <div className="flex items-baseline justify-between gap-1 tabular-nums border-t border-border/50 pt-0.5 mt-0.5">
                              <span className="text-meta text-muted-foreground font-semibold">{METRIC_SHORT[metric]}</span>
                              <span className={cn(
                                'text-label font-bold',
                                m![metric] > 0
                                  ? 'text-emerald-600 dark:text-emerald-400'
          ...
          <button
            type="button"
            onClick={onOpenDrilldown}
            className="w-full flex items-center justify-center gap-1 py-2 rounded-md text-label font-medium text-primary hover:bg-primary/5 active:bg-primary/10 transition-colors"
          >
    ...
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent align="end" className="w-[300px] p-3 space-y-3">
        <div className="space-y-1.5">
          <label className="text-data font-medium uppercase tracking-wide text-muted-foreground">De</label>
        ...
        <div className="space-y-1.5">
          <label className="text-data font-medium uppercase tracking-wide text-muted-foreground">Até</label>
          <div className="grid grid-cols-2 gap-2">
            <select
              value={toMonth}
        ...
        <p className="text-data text-muted-foreground leading-snug">
  ...
  return (
    <Popover open={open} onOpenChange={(o) => !disabled && setOpen(o)}>
      <PopoverTrigger asChild>
        ...
        >
          <span className="text-data font-medium uppercase tracking-wider text-muted-foreground">
            {label}
          </span>
